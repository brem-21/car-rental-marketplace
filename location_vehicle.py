...
locations_df = spark.read.csv(
    input_path + "locations.csv", header=True, inferSchema=True
)
rentals_df = spark.read.csv(
    input_path + "rental_transactions.csv", header=True, inferSchema=True
)
...
revenue_per_location = (
    rentals_df.groupBy("pickup_location")
...
max_min_transaction_per_location = (
    rentals_df.groupBy("pickup_location")
    .agg(
...
rental_duration = rentals_df.withColumn(
    "rental_hours",
    (col("rental_end_time").cast("long") - col("rental_start_time").cast("long"))
    / 3600,
)

rental_duration_by_vehicle = (
    rental_duration.join(vehicles_df, "vehicle_id", "inner")
...
revenue_per_location.write.parquet(
    base_output_path + "revenue_per_location", mode="overwrite"
)

logging.info(
    f"Writing transactions per location to {base_output_path}transactions_per_location"
)
transactions_per_location.write.parquet(
    base_output_path + "transactions_per_location", mode="overwrite"
)

logging.info(
    f"Writing avg transaction per location to {base_output_path}avg_transaction_per_location"
)
avg_transaction_per_location.write.parquet(
    base_output_path + "avg_transaction_per_location", mode="overwrite"
)

logging.info(
    f"Writing max/min transaction per location to {base_output_path}max_min_transaction_per_location"
)
max_min_transaction_per_location.write.parquet(
    base_output_path + "max_min_transaction_per_location", mode="overwrite"
)

logging.info(
    f"Writing unique vehicles per location to {base_output_path}unique_vehicles_per_location"
)
unique_vehicles_per_location.write.parquet(
    base_output_path + "unique_vehicles_per_location", mode="overwrite"
)

logging.info(
    f"Writing rental duration by vehicle to {base_output_path}rental_duration_by_vehicle"
)
rental_duration_by_vehicle.write.parquet(
    base_output_path + "rental_duration_by_vehicle", mode="overwrite"
)
